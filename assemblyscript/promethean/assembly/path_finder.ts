...
class PathFinderGraph{
    ...
    constructor(height: i32, width: i32){
        ...
        this.m_width = width;
        this.m_internal_grid = new StaticArray<StaticArray<PathFinderNode>>(height);
        for(let x = 0, x_len = height; x < x_len; x++){
            ...
            unchecked(this.m_internal_grid[x] = x_array);
        }
...
export class PathFinder{
    ...
    constructor(world_grid: StaticArray<StaticArray<PathFinderTile>>, options: PathFinderOptions){
        ...
        this.m_graph = new PathFinderGraph(this.m_height, this.m_width);
    }

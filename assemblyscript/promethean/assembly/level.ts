...
export class Level{
    ...
    constructor(height: i32, width: i32){
        ...
        this.m_width = width;
        this.m_level = new StaticArray<StaticArray<Tile>>(height);
        for(let x = 0; x < height; x++){
            let level_x = new StaticArray<Tile>(width);
            for(let y = 0; y < width; y++){
                unchecked(level_x[y] = Tile.Empty);
            }
            unchecked(this.m_level[x] = level_x);
        }
        this.m_statistics = new LevelStatistics();
    }

    set_statistics(rooms_count: i32, corridors_count: i32, all_corridors: bool, room_centers: StaticArray<Point>): void{
